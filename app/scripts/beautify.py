...
def draw_filled_rounded_rect(image_np, pt1, pt2, color_bgr, radius,
    ...
    # 绘制矩形部分 (更精确地绘制，避免在部分圆角为False时出现间隙)
    # 顶部矩形
...
def draw_bordered_rounded_rect(image_np, pt1, pt2, color_bgr, thickness, radius,
    ...
    # 绘制直线部分 (更精确地绘制，避免在部分圆角为False时出现间隙)
    # 顶部横线
    cv2.line(image_np,
            (x1 + (radius if top_left_round else 0), y1),
...
def custom_plot(
        ...
        LABEL_MAPPING=None,
        COLOR_MAPPING=None,  # 新增参数，用于自定义颜色映射
        beautify=True
):
    """绘制检测框和标签 (始终执行美化模式)"""
    ...
    for box, conf, label_key in zip(boxes, confs, labels):
        ...
        color_bgr = COLOR_MAPPING.get(int(label_key), (0, 255, 0))  # 默认绿色

        # 标签语言
        if use_chinese_mapping:
            display_label = LABEL_MAPPING.get(int(label_key), label_key)
        else:
            display_label = LABEL_MAPPING.get(int(label_key), label_key)
